...
// 交互事件处理部分
  ...
  var item1 = '请选择省份';
  var item2 = '请选择城市';
  var urlUp = '/static/assets/page-views/cooperation/cooperation-apply/Fill1.png';
  var urlDown = '/static/assets/page-views/cooperation/cooperation-apply/Fill1-1.png';
  var id = -1;//默认值即未选择省份前id设为-1，用来冻结city箭头改变

  $('.province span').text(item1);
  $('.city span').text(item2);
  $.each(province, function (index, item) {
    $('.Pwrapper').append('<div class=Pcategory' + ' id=' + index + '>' + item['provinceName'] + '</div>');
  });


  function provinceClickHandle() {
    if (!$('.Cwrapper').is(':hidden')) {
      $('.Cwrapper').slideToggle(1);
  ...
  $('.Pcategory').click(function (e) {
    ...
    $('.Ccategory').remove();
    id = $(this).attr('id');
    item2 = province[id]['citys'][0]['citysName'];
    $('.city span').text(item2);
    $.each(province[id]['citys'], function (index, item) {
      $('.Cwrapper').append('<div class=Ccategory>' + item['citysName'] + '</div>');
    });
